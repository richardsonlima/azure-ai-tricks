...
""" Ensure you have streamlit, azure-storage-blob, azure-ai-openai, 
and azure-search-documents libraries installed. 
"""
...
curation_rules = st.text_area("Enter Curation Rules")
context = st.text_area("Enter Context for LLM")
...
if st.button("Curate Knowledge Base and Upload"):
    if stack_name and content_type_name and knowledge_base and curation_rules and context and embedding_model_name and index_name and json_fields:
        openai_client = OpenAIClient(endpoint=f"https://{embedding_model_name}.openai.azure.com/", credential=AzureKeyCredential(OPENAI_API_KEY))
        
        # Curate knowledge base
        prompt_template = f"Context: {context}\nCuration Rules: {curation_rules}\nContent: {{content}}\n"
        curated_content = curate_knowledge_base(knowledge_base, openai_client, prompt_template)
